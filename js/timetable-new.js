 ...
 **/
 ...
 (function ($, Drupal) {
   ...
   Drupal.behaviors.gladiolen = {
     attach: function (context, settings) {
       Drupal.gladiolen_detectTouchDevice();
       Drupal.gladiolen_watchResize();
       Drupal.gladiolen_mobileEvents();
     }
   };
 
   Drupal.gladiolen_detectTouchDevice = function () {
   ...
   Drupal.gladiolen_mobileEvents = function () {
     $(".mobile-menu-open").click(mobileMenuHandler);
     $(".mobile-menu-close").click(mobileMenuHandler);
   };
   ...
   Drupal.gladiolen_watchResize = function () {
     $(window).bind("resize", listenWidth).resize();
   };
 ...
 (function ($, Drupal) {
   ...
   Drupal.behaviors.pager = {
     attach: function (context) {
       ...
       $('.pager-next').once('bind-pager-next').click(function (evt) {
       ...
       $('.pager-prev').once('bind-pager-prev').click(function (evt) {
         evt.preventDefault();
 ...
 (function ($, Drupal) {
   ...
   Drupal.tijdslotlike = Drupal.tijdslotlike || {};
 
   /**
    * Triggered on click like.
    */
   Drupal.tijdslotlike.toggle_like = function () {
     var timeslot_id = String($(this).closest('.timeslot-like-wrapper').data('timeslot-id'));
 
     if (Drupal.tijdslotlike.likes.items().indexOf(timeslot_id) != -1) {
       Drupal.tijdslotlike.likes.remove(timeslot_id);
     } else {
       Drupal.tijdslotlike.likes.add(timeslot_id);
     }
     Drupal.tijdslotlike.update_likes();
   };
 
   /**
    * Rebuild likes classes.
    */
   Drupal.tijdslotlike.update_likes = function () {
     $('.timeslot-like-wrapper').removeClass('liked');
     var arrayLength = Drupal.tijdslotlike.likes.items().length;
     for (var i = 0; i < arrayLength; i++) {
       var tijdslot_id = Drupal.tijdslotlike.likes.items()[i];
       $('.timeslot-like-wrapper[data-timeslot-id="' + tijdslot_id + '"]').addClass('liked');
     }
   };
   ...
   $(document).ready(function () {
     ...
     // Init likes.
     Drupal.tijdslotlike.likes = new cookieList("gladiolen-timeslot-likes");
 
     // Show current likes.
     Drupal.tijdslotlike.update_likes();
 
     // Bind like toggle.
     $('.timeslot-like-button').once('bind-like').each(function () {
       $(this).click(Drupal.tijdslotlike.toggle_like);
     });
 
   });
 
   /**
    * StackOverflow ftw (https://stackoverflow.com/questions/1959455/how-to-store-an-array-in-jquery-cookie)
    */
